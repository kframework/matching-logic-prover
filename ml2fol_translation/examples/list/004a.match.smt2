...
(assert (forall ((x Nat) (y Nat) (s NatSeq))
  (=> (or (delta_mapstoseq x (cons y s)) (and (delta_merge (total_mapsto x y) (total_mapstoseq (succ x) s)) (delta_mapsto x y) (delta_mapstoseq (succ x) s))) 
...
; (smt.stats :time 212.77 :before-memory 3.05 :after-memory 172.01)
