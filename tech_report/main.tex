...
\begin{thm}
	For any matching logic theory $\Gamma$ and a pattern $P$,
	\begin{equation*}
...
The formula(s) that are autogenerated by $\MLFOL$ are often unnecessarily verbose. Quantified dummy variables are everywhere, which makes it too hard for SMT solvers. Therefore, it is necessary to preprocess and simplify the autogenerated formulas before throwing them to solvers. In the next paragraph, we will introduce nine simplification rules that are crucial and useful in practice. All of them are trivially valid simplification rules, so the emphasis will be put on the reason why we apply them.
...
The author did some experiments with the translation 
