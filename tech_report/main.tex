...
\newcommand{\inferrule}[1]{\text{\scriptsize{(#1)}}}
...
\begin{proof}
$$
\infer[\inferrule{MP}]{\vdash Q[P_2 / x]}
{\infer[\inferrule{MP}]{\vdash Q[P_1 / x] \to Q[P_2 / x]}
	   {\infer{P_1 = P_2}{\cdot}
	   &\infer[\inferrule{K6}]{P_1 = P_2 \to \left(Q[P_1 / x] \to Q[P_2 / x]\right)}{\cdot}}
...
\begin{proof}
That the right hand side implies the left is easy, so we only prove that the left implies the right. By definition, $\left(P = Q\right) = \neg \ceil{\neg \left(P \leftrightarrow Q\right)}$
\end{proof}
