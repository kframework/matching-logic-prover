...
Given a matching logic signature $(S, \Sigma)$ where $\Sigma = \Sigma_{\textit{func}} \cup \Sigma_{\textit{partial}} \cup \Sigma_{\textit{else}}$ contains some functional symbols, some partial symbols and some other symbols. Define two formula transformations $\mathsf{fol}$ and $\mathsf{fol_2}$ as follows.


\begin{algorithm}
	\KwIn{a matching logic pattern $P$}
	\KwOut{a first order formula}
	generate a fresh variable $r$\;
...
\begin{defn}
	A theory of symbolic execution is a theory of transition system whose axioms are all of the form $P(\vec{x}, \vec{z}) \Rightarrow^{\forall}_1 Q(\vec{x}, \vec{y})$ where 
\end{defn}
