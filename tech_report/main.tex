...
\end{lemma}
\begin{proof}
	There is little intelligence in the proof. We will show how to prove (1) as an example, and the rest are left as exercises. 
	The derivation tree
	$$
	\infer[\rl{Lemma~\ref{lemma:ereplacement}}]{\vdash P}
	{ \infer{\vdash P=\top}{\cdot}
		& \infer{\vdash \top}{\cdot}}
	$$
	shows that the right implies the left. The show the other direction, we have the following derivation tree
	$$
	\infer{\vdash P = \top}
	{\infer{\vdash \neg \ceil{\neg (P \leftrightarrow \top)}}
	{\infer{\vdash \forall x . (x \in \neg \ceil{\neg (P \leftrightarrow \top)})}
	{\infer{\vdash x \in \neg \ceil{\neg (P \leftrightarrow \top)}}
	{\infer{\vdash \neg (x \in \ceil{\neg (P \leftrightarrow \top)})}
	{\infer{\vdash \neg \exists y . (x \in \ceil{y} \wedge y \in (\neg (P \leftrightarrow \top)))}
	{\infer{\vdash \forall y . \neg (x \in \ceil{y} \wedge y \in (\neg (P \leftrightarrow \top)))}
	{\infer{\vdash \neg (x \in \ceil{y} \wedge y \in (\neg (P \leftrightarrow \top)))}
	{\infer{\vdash (\neg (x \in \ceil{y})) \vee (\neg (y \in (\neg (P \leftrightarrow \top))))}
	{\infer{\vdash (\neg (x \in \ceil{y})) \vee (\neg \neg (y \in (P \leftrightarrow \top)))}
	{\infer{\vdash (\neg (x \in \ceil{y})) \vee (y \in (P \leftrightarrow \top))}
	{\infer{\vdash y \in (P \leftrightarrow \top)}
	{\infer{\vdash y \in P \leftrightarrow y \in \top}
	{\infer{\vdash y \in P \to y \in \top}
	{\vdash y \in \top}
	&\infer{\vdash y \in \top \to y \in P}
	{\vdash y \in P}}}}}}}}}}}}}}.
	$$
	What is left is to prove if $\vdash P$, then $\vdash y \in P$, as shown in the next derivation tree.
	$$
	\infer{\vdash y \in P}
	{\infer{\vdash \forall x . (x \in P)}
		{\infer{\vdash P}{\cdot}}
	&\infer{\vdash \forall x . (x \in P) \to y \in P}{\cdot}
	}.
	$$
\end{proof}
...
{\infer{\vdash \neg (x \in \ceil{y}) \vee (y \in P \leftrightarrow Q)}
{\infer{\vdash y \in P \leftrightarrow Q}
{}}}}}}}}.
$$
