...
seem to be the most popular one [5+ citations]. 
In 2014, [who] initiated the first edition of separation logic competition (SL-COMP) alongside the satisfiability modulo theories competition (SMT-COMP), and successfully held the second edition in 2015. 
Six(?) state-of-the-art separation logic provers participated in the events and competed on a set of benchmarks~\cite{}. 
The benchmarks include hundreds of separation logic formulas of a fragment of SLRD, namely XXX (SLL?), in which ....
...
\end{equation*}
Separation logic assertions are often written in an extended SMT-LIB format~\cite{}. 
...
Here we only provide a brief review of SLRD. Readers are refered to~\cite{} for more details.
...
\begin{align*}
&f \in \FF \ \text{field names} & &\emph{P} \in \PP \ \text{recursive definition name}\\
...
$\FF$ is the set of field names, which is defined using the \textit{Field} sort in the theory. Each field is declared as a function symbol of arity 0 and result type \textit{Field\ A\ B}. \textit{A} is the sort corresponding to the record type declaring the field and \textit{B} is the sort typing the field. The following is an example of the definition of a binary tree node.
\begin{verbatim}
...
\begin{verbatim}
(declare-sort Sll_t)
;it's used just to make the formula look the same as above
...
(declare-func sref (Record) Record)
