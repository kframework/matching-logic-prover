...
\title{Technical Report: Deduction in Matching Logic}
...
This paper discusses deduction in matching logic.
...
&x \in_{s_1}^{s_2} P \coloneqq \ceil{x \wedge P}_{s_1}^{s_2}.
\end{align*}
...
\item (M5) $x \in \sigma(\dots,P_i,\dots) = \exists y . y \in P_i \wedge x \in 
\sigma(\dots,y,\dots)$ where $y$ does not occur free in the left-hand side of 
the equation.
\end{itemize}
...
\section{A sound and complete proof system for the zeroth-order fragment of 
matching logic}

The purpose of this section is to introduce a proof system for the zeroth-order 
fragment of matching logic and to establish its soundness and completeness.
Unlike the proof system we have seen in Section~\ref{sec:deductive_system}, the 
proof system that we define in this section does not rely on the presence of 
the definedness symbols.

The zeroth-order fragment of matching logic is matching logic without variables 
and quantifiers.
Given a signature $\Sigma$ which contains at least a zero-arity symbol for each 
sort, the syntax of zeroth-order matching logic is defined as follows.
\begin{align}
P \Coloneqq\  & P_1 \wedge P_2 \nonumber \\
       \mid\  & \neg P \nonumber \\
       \mid\  & \sigma(P_1,\dots,P_n). \nonumber
\end{align}
Disjunction, implication, double implication, top and bottom are defined in 
their usual way as syntactic sugar.

The proof system of zeroth-order matching logic consists of five axiom 
schemas and two inference rules.

\begin{itemize}
\item (P1) $P \to (Q \to P)$
\item (P2) $(P \to (Q \to R)) \to (P \to Q) \to (P \to R)$
\item (P3) $(\neg Q \to \neg P) \to (P \to Q)$
...
\item (Congruence) From $P \leftrightarrow Q$, deduce $R[P] \leftrightarrow 
R[Q]$.
\end{itemize}

We will show the above proof system is sound and complete for zeroth-order 
matching logic.
It might not be the minimal one as one may get rid of the (Congruence) rule and 
remain complete.
The focus here is to find a proof system that is independent of the definedness 
symbols.
Proving soundness is easy and we will skip it.
 We will prove its completeness using the standard approach of 
{maximal consistent sets construction} from which we can build a {canonical 
model} for any consistent sets of axioms.
The high-level proof sketch is as follows.
Assume $\Gamma$ is consistent and $\Gamma \not\vdash \varphi$.
We can show that $\Gamma \cup \{\neg \varphi\}$ is also consistent.
Then we show that we can construct a canonical model $M$ of the consistent set
$\Gamma \cup \{ \neg \varphi \}$, and this shows $\Gamma \not \vDash \varphi$.
...
\section{Minimal Proof System}
Matching logic is proved to have a sound complete proof system in the presence 
of definedness symbols.
We aim at a sound and complete proof system that does not 
rely on definedness symbols.
We conjecture the following proof system~$\miniPS$ is such a proof system.
\begingroup
\renewcommand\arraystretch{1.3}
	\begin{longtable}{ll}
		...
		\\
		\textsc{$\wedge$}
		&  $\sigma(\dots,\varphi_1 \wedge \varphi_2,\dots) 
		\imp
		\sigma(\dots,\varphi_1,\dots) \wedge \sigma(\dots,\varphi_2,\dots)$
		\\
		\textsc{N}
		&  $\neg \sigma(\bot)$
		\\
		...
		&  From $\varphi_1 \dimp \varphi_2$, deduce $\psi[\varphi_1 / x] \dimp 
		\psi[\varphi_2 / x]$
		\\
		\textsc{Framing}
		& From $\varphi_1 \imp \varphi_2$, deduce $\sigma(\dots, 
		\varphi_1,\dots) \imp \sigma(\dots, \varphi_2, \dots)$
		\\
		\textsc{Implication Introduction}
		& From $\varphi$ deduce $x \imp \varphi$, where $x$ does not occur free 
		in $\varphi$
		\\
		\textsc{Implication Elimination}
		& From $x \imp \varphi$ deduce $\varphi$, where $x$ does not occur free 
		in $\varphi$
		\\
		\caption{A proof system without the presence of definedness symbols}
		\end{longtable}
\endgroup

We confess the proof system $\miniPS$ shown above is not minimal yet.
Many axioms seem to be derivable.
However, for now we mainly focus on finding a proof system that is independent 
of the definedness symbols.
We will prove that $\miniPS$ is sound and complete, but before that, let us 
establish some useful metatheorems of $\miniPS$.
This shall give us some confidence that $\miniPS$ is indeed complete.

\begin{prop}[Tautology] \label{prop:taut}
	For any propositional tautology $\mathcal{A}(p_1,\dots,p_n)$ where 
	$p_1,\dots,p_n$ are all propositional variables in $\mathcal{A}$, and for 
	any patterns $\varphi_1,\dots,\varphi_n$ such that the pattern
	$\mathcal{A}(\varphi_1,\dots,\varphi_n)$ is wellformed,
	$$ \vdash 
	\mathcal{A}(\varphi_1,\dots,\varphi_n).$$
\end{prop}
\begin{proof}
	Simply mimic the proof in propositional logic.
\end{proof}
...
\end{prop}
\begin{proof}
	Mimic the proof in predicate logic.
\end{proof}

We say two patterns $\varphi_1$ and $\varphi_2$ are \emph{provably equivalent} 
in $\Gamma$, if $\Gamma\vdash \varphi_1$ iff $\Gamma \vdash \varphi_2$.
We often omit $\Gamma$ if it is clear from the context.
We use $\varphi_1 \equiv \varphi_2$ to denote that $\varphi_1$ and $\varphi_2$ 
are probably equivalent.
One can easily verify that if $\Gamma \vdash \varphi_1 \dimp \varphi_2$, then 
$\varphi_1 \equiv \varphi_2$ in $\Gamma$.
The notation of probably equivalence is not essential, but it makes the proof 
of many metatheorems more succinct and intuitive.


\begin{prop}
	In the presence of definedness symbols, 
	$\vdash \varphi = \varphi$.
\end{prop}
\begin{proof}
	By definition, 
	$\varphi = \varphi \equiv \neg \ceil{\neg (\varphi \dimp \varphi)}$, which 
...
\begin{prop}
	In the presence of definedness symbols,
	$\vdash \varphi_1 \dimp \varphi_2$ iff $\vdash \varphi_1 = \varphi_2$.
\end{prop}
\begin{proof}
	Use (Equational Substitution).
\end{proof}


The following transformation is often useful.
\begin{align*}
\varphi_1 = \varphi_2
&\equiv \neg \ceil{\neg(\varphi_1 \dimp \varphi_2)} \\
&\equiv \neg \ceil{(\varphi_1 \wedge \neg \varphi_2) \vee (\neg \varphi_1 
\wedge \varphi_2)} \\
&\equiv \neg \left( \ceil{\varphi_1 \wedge \neg \varphi_2}  \vee  \ceil{\neg 
\varphi_1 \wedge \varphi_2} \right) \\
&\equiv \neg \ceil{\varphi_1 \wedge \neg \varphi_2} \wedge \neg \ceil{\neg 
...
\begin{itemize}
	\item (Case 1). $\ceil{x \wedge y} \imp \neg \ceil{x \wedge \neg y} \wedge 
	\neg \ceil{\neg x \wedge y}$
	\item (Case 2). $\neg(\ceil{x \wedge \neg y} \vee \ceil{\neg x \wedge y}) 
	\imp \ceil{x \wedge y}$
\end{itemize}
Notice that by the (N) rule and the axiom $\forall x . \ceil{x}$, we can prove 
that
\begin{align*}
& \ceil{x \wedge y} \vee \ceil{x \wedge \neg y} \equiv \ceil{x} \equiv \top \\
& \ceil{x \wedge y} \vee \ceil{\neg x \wedge y} \equiv \ceil{y} \equiv \top
\end{align*}
We leave the readers to verify that (Case 2) can be easily established.
To prove (Case~1), notice its symmetric form, and we only need to prove
\begin{align*}
\ceil{x \wedge y} \imp \neg \ceil{x \wedge \neg y}
&\equiv 
\end{align*}
...
% PLEASE FOLLOW FSL RULES. FOR EXAMPLE, EACH SENTENCE IN ONE LINE, ETC.
...
We have \textit{junk} that is true for any heap, whether empty or consisting of some allocated nodes. We have * as the separating conjunction symbol. %What's more, note that we have pure formulas typed by the \textit{Bool} sort and spatial formulas typed by the \textit{Space} sort, the spatial atoms' combination with a pure formula requires to cast \textit{Space} to \textit{Bool} using the symbol \textit{tobool}. 
...
Over 400 translated examples can be refered in this repository.
...
In the following codes, %symbol \textit{ssep} denotes the separating conjunction symbol, it can take arbitrary number of \textit{Maps} as input. Symbol \textit{pto} denotes $\mapsto$. %and \textit{tobool} is used to cast spatial formulas to pure ones. %We use \textit{ref} to denote a field reference and \textit{sref} to denote a set of field references.
...
The following is the matching logic version of the above example. It shall be noted that proving assertion $P$ is equivalent to establishing \textit{not(floor\ P)} is unsat. Its syntax refers to the matching logic syntax used in section 2.3.
\begin{verbatim}
(declare-sort Sll_t)
(declare-sort Record)
(declare-sort Map)
...
This section studies the connection between matching logic and linear temporal 
logic.

Consider a unary symbol $\sigma$.
Given a model $M$, the symbol $\sigma$ is interpreted as a function
$$ \sigma_M \colon M \to 2^M $$
Notice that there is an isomorphism between the sets $M \to 2^M$ and $2^{M 
\times M}$.
The isomorphism implies the following two facts.
Firstly, every interpretation of a symbol can be seen as 
a binary relation on the domain.
Secondly, every binary relation on the domain can be taken as an interpretation 
of a symbol.
This is formalized in the next definition.
\begin{defn}
Given a function $\sigma_M \colon M \to 2^M$, the binary relation $R_\sigma$ 
deduced from $\sigma_M$ is defined as
\begin{center}
$(a,b) \in R_\sigma$ \quad iff \quad $a \in \sigma_M(b)$
\end{center}
\end{defn}

\begin{eg}
Given a signature $(S, \Sigma)$ where $S = \{s\}$ and $\Sigma = \{\circ\}$.
The symbol $\circ$ is known as the ``next'' operator.
Given a model $M$ of the signature and an interpretation of $\circ$:
$$ \circ_M \colon M \to 2^M $$
The binary relation deduced from $\circ_M$, denoted as $\Rightarrow$, is 
defined as
\begin{center}
$a \Rightarrow b$ \quad iff \quad $a \in \circ_M(b)$
\end{center}
\end{eg}

Duality is an important property of unary constructs.
The dual of a construct $c$ is denoted and defined as
$$ \bar{c} = \neg c \neg $$
As an example, it is well-known that the universal quantifier $\forall x$ and 
the existential quantifier $\exists x$ have each other as dual constructs:
\begin{align*}
\forall x . \varphi = \neg \exists x . (\neg \varphi) \\
\exists x . \varphi = \neg \forall x . (\neg \varphi)
\end{align*}
Negation is dual to itself:
$$\neg \varphi = \neg (\neg (\neg \varphi))$$
For a symbol $\sigma$, its dual $\bar{\sigma}$ is
$$ \bar{\sigma}(\varphi) = \neg \sigma (\neg \varphi)
$$
Notice that in general, the dual of a symbol $\bar{\sigma}$ is not 
``interpreted'' 
as a binary relation as the symbol $\sigma$ does.
Given a model $M$, a construct $c$ is ``interpreted'' as a function
$$ c_M \colon 2^M \to 2^M $$
This is because the construct $c$ can take any pattern $\varphi$ and gives back 
another pattern $c(\varphi)$, which is obtained by applying $c$ to $\varphi$.
As a result, ``interpretation'' of unary constructs cannot be naturally viewed 
as binary relations.

\begin{defn}
Given a construct $c$ and a model $M$. We say $c$ is a point-wise extension 
in $M$, if the function $c_M \colon 2^M \to 2^M$ satisfies 
the following disjunctive propagation conditions:
\begin{itemize}
\item $c_M(\emptyset) = \emptyset$
\item $c_M(A \cup B) = c_M(A) \cup c_M(B)$, for any $A, B \subseteq M$
\end{itemize}
This means the function $c_M$ is fully determined by its values on singleton 
sets:
$$
c_M(A) = \bigcup_{a \in A} c_M(\{a\})
$$
\end{defn}

\begin{thm}
Given a unary symbol $\sigma$ and a model $M$.
The following statements are equivalent:
\begin{enumerate}
\item The dual $\bar{\sigma}$ is a point-wise extension in $M$;
\item $\sigma_M(M) = M$ and $\sigma_M(A \cap B) = \sigma_M(A) \cap \sigma_M(B)$
\item The binary relation $R_\sigma$ deduced from $\sigma_M$ is a function;
\item $\sigma_M = \bar{\sigma}_M$
\end{enumerate}
\end{thm}
\begin{proof}
($1 \Rightarrow 2$).
By definition, $\bar{\sigma}_M(\emptyset) = M \setminus \sigma_M(M \setminus 
\emptyset) = M \setminus \sigma_M(M ) = \emptyset$, so $\sigma_M(M) = M$.
Similarly, $\bar{\sigma}_M(A \cup B) = M \setminus \sigma_M(M \setminus (A \cup 
B)) = M \setminus \sigma_M((M \setminus A) \cap (M \setminus B)) = 
\bar{\sigma}_M(A) \cup \bar{\sigma}_M(B) = (M \setminus \sigma_M(M \setminus 
A)) \cup (M \setminus \sigma_M(M \setminus B)) = M \setminus (\sigma_M(M 
\setminus 
A) \cap \sigma_M(M \setminus 
B))$, so $\sigma_M((M \setminus A) \cap (M \setminus B)) = (\sigma_M(M 
\setminus 
A) \cap \sigma_M(M \setminus 
B))$. Because $A, B$ are arbitrary subsets, this gives us $\sigma_M(A \cap B) = 
\sigma_M(A) \cap \sigma_M(b)$.

($2 \Rightarrow 3$).
Because $\sigma_M(M) = M$, for any $a$, there exists a $b$ such that $(a, b) 
\in R_\sigma$.
Assume there exist $a, b_1, b_2$ and $b_1 \neq b_2$ such that $(a, b_1), (a, 
b_2) \in R_\sigma$.
By definition, $a \in \sigma_M(b_1)$ and $a \in \sigma_M(b_2)$, so $a \in 
\sigma_M(b_1) \cap \sigma_M(b_2) = \sigma_M(\{b_1\} \cap \{b_2\}) = 
\sigma_M(\emptyset) = \emptyset$.
This is a contradiction, which means our assumption is incorrect.
Therefore, for any a, there exists one and exactly one $b$ such that $(a, b) 
\in R_\sigma$, and thus $R_\sigma$ is a function.

($3 \Rightarrow 4$).
We only show that for any $b$, $\sigma_M(b) \subseteq \bar{\sigma}_M(b)$.
The other direction can be obtained by considering $\bar{\sigma}_M$ and 
$\bar{\bar{\sigma}}_M$, and the fact that $\sigma_M = \bar{\bar{\sigma}}_M$.
Now let $a \in \sigma_M(b)$,
\begin{align*}
&a \in \bar{\sigma}_M(b) \\
\text{if}\quad & a \not\in \sigma_M(M \setminus \{b\}) \\
\text{if}\quad & a \notin \sigma_M(c) \quad \text{for any $c \neq b$} \\
\text{if}\quad & a \in \sigma_M(c) \text{ implies }  c = b \quad \text{for any 
$c$} \\
\text{if}\quad & (a, c) \in R_\sigma \text{ implies }  c = b \quad \text{for 
any 
$c$}
\end{align*}
The last statement is true, because $R_\sigma$ is a function, and by assumption 
we have $a \in \sigma_M(b)$, so $(a,b) \in R_\sigma$.
\end{proof}
...
\begin{prop}
For any unary symbol $\circ$, 
\begin{displaymath}
\prftree
{\circ\varphi \dimp \neg \circ \neg \varphi}
{\circ(\varphi \to \psi) \to (\circ \varphi \to \circ \psi)}
\end{displaymath}
\end{prop}
...
\bibitem{SymbolicExecutionwithSeparationLogic} 
Berdine J, Calcagno C, O'Hearn P W. Symbolic execution with separation 
logic[C]//APLAS. 2005, 5(3780): 52-68.
