...
$c \top \and \or $.
$c \ceil \floor \eq \member \subset $.
$c \domain $.

top-pattern $a #Pattern \top $.
and-pattern $a #Pattern ( \and ph ps ) $.
or-pattern $a #Pattern ( \or ph ps ) $.

ceil-pattern   $a #Pattern ( \ceil ph ) $.
floor-pattern  $a #Pattern ( \floor ph ) $.
eq-pattern     $a #Pattern ( \eq ph ps ) $.
member-pattern $a #Pattern ( \member ph ps ) $.
subset-pattern $a #Pattern ( \subset ph ps ) $.

domain-pattern $a #Pattern ( \domain ph ) $.

definedness $a |- ( \ceil x ) $.
...
ex-s-pattern $a #Pattern ( \ex-s x ph ps ) $.
fa-s-pattern $a #Pattern ( \fa-s x ph ps ) $.

$(
    Rewrite symbols and axioms
$)

$c \rewrites $.

rewrites-pattern $p #Pattern ( \rewrites ph ps ) $= ? $.

${
    rewrites-subst.1 $e #Substitution ph3 ph1 ps xX $.
    rewrites-subst.2 $e #Substitution ph4 ph2 ps xX $.
    rewrites-subst   $p #Substitution ( \rewrites ph3 ph4 ) ( \rewrites ph1 ph2 ) ps xX $= ? $.
$}

$( TODO: is this correct? $)
${
    fa-s-elim.1 $e |- ( \ex-s x ph1 ( \eq ph2 x ) ) $.
    fa-s-elim.2 $e |- ( \fa-s x ph1 ph3 ) $.
    fa-s-elim.3 $e #Substitution ph4 ph3 ph2 x $.
    fa-s-elim   $p |- ph4 $= ? $.
$}

$( -------------------------------------------------------------------------- $)
$( | the following section should be auto-generated                           $)
$( -------------------------------------------------------------------------- $)
...
FOO-sort-Foo-pattern $a #Pattern FOO-sort-Foo $.
FOO-symbol-foo-pattern $a #Pattern FOO-symbol-foo $.
FOO-symbol-id-pattern $a #Pattern ( FOO-symbol-id ph ) $.

$( FOO-symbol-foo axioms $)
FOO-symbol-foo-function $a |- ( \ex-s x FOO-sort-Foo ( \eq FOO-symbol-foo x ) ) $.
FOO-symbol-foo-subst $p #Substitution FOO-symbol-foo FOO-symbol-foo ph xX $= ? $.

$( FOO-symbol-id axioms $)
FOO-symbol-id-function $a |- ( \fa-s x FOO-sort-Foo ( \ex-s y FOO-sort-Foo ( \eq ( FOO-symbol-id x ) y ) ) ) $.
${
    FOO-symbol-id-subst.1 $e #Substitution ph2 ph1 ph xX $.
    FOO-symbol-id-subst $p #Substitution ( FOO-symbol-id ph2 ) ( FOO-symbol-id ph1 ) ph xX $= ? $.
$}

FOO.rule.1 $a |- ( \fa-s x FOO-sort-Foo ( \rewrites ( FOO-symbol-id x ) x ) ) $.

$( Step 1 $)

FOO.step.1 $p |- ( \rewrites ( FOO-symbol-id FOO-symbol-foo ) FOO-symbol-foo ) $=
  FOO-sort-Foo-pattern FOO-symbol-foo-pattern vx wev FOO-symbol-id-pattern vx
  wev rewrites-pattern FOO-symbol-foo-pattern FOO-symbol-id-pattern
  FOO-symbol-foo-pattern rewrites-pattern vx vx FOO-symbol-foo-function vx
  FOO.rule.1 FOO-symbol-foo-pattern vx wev FOO-symbol-id-pattern vx wev
  FOO-symbol-foo-pattern FOO-symbol-id-pattern FOO-symbol-foo-pattern vx vev
  FOO-symbol-foo-pattern vx wev FOO-symbol-foo-pattern vx vev
  FOO-symbol-foo-pattern vx vev sb FOO-symbol-id-subst FOO-symbol-foo-pattern
  vx vev sb rewrites-subst fa.s-elim $.
